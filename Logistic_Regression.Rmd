...
# attr(mydata$PTSD_Current, "ATT") <- NULL
# str(mydata)
# 
mydata <- na.omit(mydata)
# labels<-names(mydata)[2:length(mydata)]
```
...
ggplot2::ggplot(RW.Results, aes(x=reorder(Variables,Rescaled.RelWeight),y=Rescaled.RelWeight)) +
  # geom_hline(yintercept = 1,linetype="dashed",alpha = .25) +
  # geom_hline(yintercept = .5,linetype="dashed",alpha = .25) +
  geom_bar(stat = "identity", color = "Black") +
  ggtitle("Relative weight (importance) of predictors on Physical Activity") +
  theme_classic() +
  xlab('Variable')+
  ylab('% of importance in physical activity')+
  ylim(0,40)+
  geom_text(aes(label = Rescaled.RelWeight), vjust =.5, hjust = -.5, color = "black")+
  coord_flip() 
...
library(broom)
CI.Results<- tidy(logBoot,conf.int=TRUE,conf.method="perc") 
write_csv(CI.Results, "~/Downloads/CI.Results")
# runBoot(length(mydata[,2:numVar]))

# # runBoot<-function(num){
# 
# INDEX<-1:length(mydata[,2:numVar])
# INDEX
# test<-lapply(INDEX, FUN=mybootci)
# test
# test2<-t(sapply(test,'[[',i=4)) #extracts confidence interval
...
rand_CI<- tidy(logBoot,conf.int=TRUE,conf.method="perc")
