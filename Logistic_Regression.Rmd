...
fit <- glm(MET_min~
             ...
             TOTAL_TRAUMAS + BMI + 
             #Physical Health Conditions
             Athsma + Diabetes + GAD_Current + MDD_Current + DUD_Current,
             # Athsma + Chron_pain + Diabetes+ Hrt_atk + 
             # Hrt_dis + High_chol + High_bld_press + Osteoporosis + Stroke +
             # #Mental Health conditions
             # AUD_Current + DUD_Current + MDD_Current + PTSD_Current + GAD_Current,
...
ggplot2::ggplot(impo_df, aes(x=reorder(rownames(impo_df),R2), y=R2)) +
geom_point( color="blue", size=4, alpha=0.6)+
geom_segment( aes(x=rownames(impo_df), xend=rownames(impo_df), y=0, yend=R2), 
color='skyblue') +
xlab('Variable')+
