...
EX_FP <- 
  ...
  geom_linerange(aes(xmin=OR_low, xmax=OR_high),size = 1, show.legend = FALSE,
                color = ifelse(fp_df$p.adjusted < 0.05, "darkblue", "darkgrey")) +
...
wt_plot1 <-
  ...
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 16,
             size=5,show.legend = FALSE, color=
               ifelse(wt_df$res.CI.low.man < 0, "darkgrey", "darkgreen"))+
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 1,
             size=5,show.legend = FALSE,color="white")+
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 1,
             size=5.2,show.legend = FALSE,color="white")+
    geom_point(aes(x=Rescaled.RelWeight.percent), shape= 1,
             size=5.4,show.legend = FALSE,color="white")+
  #using diamond
...
wt_plot2 <-
  ...
  theme_classic() +
  #make the forrest plot
  geom_linerange(aes(xmin=res.CI.low.man, xmax=res.CI.high.man),size = 1, show.legend = FALSE,color = "darkblue")+
  geom_linerange(aes(xmin=Low2, xmax=High2),size = 2, show.legend = FALSE,color = "darkblue")+ 
  geom_linerange(aes(xmin=Low1, xmax=High1),size = 3, show.legend = FALSE, color = "darkblue")+
  #using a box
  # geom_point(aes(x=Rescaled.RelWeight.percent), shape= 15,
             # size=4,show.legend = FALSE, color="orange")+
  #using circle
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 18,
             size=5,show.legend = FALSE, color = "darkblue")+
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 5,
             size=4,show.legend = FALSE,color="white")+
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 5,
             size=4.2,show.legend = FALSE,color="white")+
    geom_point(aes(x=Rescaled.RelWeight.percent), shape= 5,
             size=4.4,show.legend = FALSE,color="white") + 
#change x axis name
  labs(
    x="Weight (%) of variance (R2o) in physical activity",
    subtitle =".        Relative Weight (%) Of Predictors On Physical Activity" ) +
...
#Military
Combat_Veteran,Years_in_Military,Retired,
#Health_stuff
...
#Filter out NA values
rf_df <- rf_df %>%
  filter(!is.na(Ex_rec), !is.na(BMI), !is.na(Any_Disability),
         !is.na(Arthritis), !is.na(Cancer), !is.na(Chron_pain), !is.na(Liv_dis),
         !is.na(Diabetes), !is.na(Hrt_dis), !is.na(Hrt_atk), !is.na(High_chol), 
         !is.na(High_bld_press),!is.na(Kid_dis), !is.na(Migrane), !is.na(MS), 
         !is.na(Osteoporosis), !is.na(Rhum_arth), !is.na(Stroke), !is.na(MDD_Current),
         !is.na(PTSD_Current), !is.na(AUD_Current), !is.na(DUD_Current), 
         !is.na(edu)) |>
  mutate(Obesity = ifelse(is.na(BMI), NA,
      ifelse(BMI >=30,1,0)),
      Obesity = as.factor(Obesity)
