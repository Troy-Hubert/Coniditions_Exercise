...
wt_plot1 <-
  ...
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 1,
             size=6,show.legend = FALSE,color="white")+
  geom_point(aes(x=Rescaled.RelWeight.percent), shape= 1,
             size=6.2,show.legend = FALSE,color="white")+
    geom_point(aes(x=Rescaled.RelWeight.percent), shape= 1,
             size=6.4,show.legend = FALSE,color="white")+
  #using diamond
...
wt_text_man + wt_plot1 + plot_layout(design = layout)
```
...
set.seed(71)
EX_RF_Model <-randomForest(Ex_rec~.,data=rf_df, mtry=best.m, ntree=500,importance = TRUE)
print(EX_RF_Model)
